...
namespace ProjectPE.Fragments
{
    public class searchFragment : Android.Support.V4.App.Fragment
    {
        ...
        {
            var frag4 = new searchFragment { Arguments = new Bundle() };
            return frag4;
        }
        ...
        //Listener for toolbar search function
        private class SearchViewExpandListern : Java.Lang.Object, MenuItemCompat.IOnActionExpandListener
        {
            private readonly IFilterable adapter;

            public SearchViewExpandListern(IFilterable adapter)
            {
        ...
        public class RecyclerAdapter : RecyclerView.Adapter, IFilterable
        {
            private List<dbExercise> exercises;
            private List<dbExercise> exercisesItems;
            ...
            {
                this.exercises = exercises.OrderBy(e => e.name).ToList();
                this.activity = activity;
                playlistNames = nameList;

                Filter = new ExerciseFilter(this);
            }
            ...
            //implement filter class for filtering the search results
            private class ExerciseFilter : Filter
            {
                private readonly RecyclerAdapter adapter;               
                ...
                {
                    var returnedObject = new FilterResults();
                    var results = new List<dbExercise>();

                    if (adapter.exercisesItems == null)
                        adapter.exercisesItems = adapter.exercises;

                    if (constraint == null)
                        return returnedObject;

                    if (adapter.exercisesItems != null && adapter.exercisesItems.Any())
                    {
                        results.AddRange(
                            adapter.exercisesItems.Where(
                                exercise => exercise.name.ToLower().Contains(constraint.ToString())));
                    }

                    returnedObject.Values = FromArray(results.Select(r => r.ToJavaObject()).ToArray());
                    returnedObject.Count = results.Count;
